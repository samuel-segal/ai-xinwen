...
def get_student_words(student_id: str) -> str:
    student_info_request = client.get_item(
        ...
        ProjectionExpression = 'student_id, known_words'
    )
...
def initialize_student_words(student_id: str, resource_name: str) -> None:
    with open(resource_name, 'r', encoding='utf-8') as file:
        known_words = file.read().splitlines()
        update_student_words(student_id, known_words)

#Necessary for a clean reset, for larger it may be more efficient
#To simply add the values on
def update_student_words(student_id: str, known_words: list[str]) -> None:
    client.update_item(
        TableName = 'student_info',
        Key = {
            'student_id': {
...
def get_student_articles(student_id: str):
    student_info_request = client.get_item(
        TableName = 'student_info',
        Key = {
            'student_id': {
        ...
        ProjectionExpression = 'student_id, translated_articles'
    )
